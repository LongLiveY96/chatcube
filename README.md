...
- 聊天输入新增「使用键盘发送按钮」设置，可在回车换行和键盘发送之间切换
- 助手消息支持长按或菜单进入原地文本选择，复制 Markdown 内容时会自动整理为更干净的纯文本
- MCP 工具未就绪时不再直接阻塞发送，会先继续对话并在后台刷新工具可用状态
- 首页「新对话」悬浮按钮调整尺寸、按压动效、阴影和底部占位，单手操作更顺手
- 主题色透明度处理统一收口，输入框、消息气泡、工具中心和滚动按钮等状态色更稳定
