...
- 接入华为账号一键登录，首次进入需登录，设置页可在头像入口处退出登录
- 温度、Top K 等高级生成参数迁移到助手设置，按助手统一管理个性化生成偏好
- 对话页底部补充 AI 生成标识，为应用上架合规做准备
- 附件上传与解析能力增强，支持更多类型附件读取正文并随消息注入请求
- 宽内容消息气泡自适应放宽，表格、代码块、HTML 和 Mermaid 内容更易阅读
- Mermaid 流程图支持直接预览、查看源码、复制和全屏查看，Markdown 预览能力进一步补齐
- OpenAI Responses 接入继续收紧，补齐工具调用、推理摘要、历史编码与分享导出
- 联网搜索、多会话并发、后台任务和助手头像展示更稳定
- 关于页新增邀测入口，README 邀测地址改为始终指向最新版本的活码
- 修复 Markdown 图片保存、图片分享内容丢失、助手 Tab 遮挡和 Responses 细节问题
