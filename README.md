...
- 文本附件支持读取正文并随消息注入请求，长文本资料可以直接作为上下文使用
- 助手消息和分享卡片在表格、代码块、HTML 等宽内容下自动放宽布局，不再被挤压
- Mermaid 流程图支持直接预览、查看源码、复制和全屏查看，渲染资源本地内置，离线也可用
- 多会话并发、后台任务和助手消息头像展示更稳定，不同会话之间不再互相串状态
- 关于页新增邀测入口，README 邀测地址改为始终指向最新版本的活码
- 赞助开发者页接入爱发电，并补齐赞助说明、用途和交流入口
